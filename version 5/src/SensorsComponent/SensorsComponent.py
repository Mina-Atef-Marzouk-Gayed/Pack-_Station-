...
class MySignals:
    def __init__(self):
        ...
        # NOTE: refill_stocks_cmd removed to match new VSI signals

        # Outputs to PLC (existing)
        ...
        # NEW: box-position and quality signals now produced by Sensors
...
LOW_THRESHOLD_TAPE = 5     # yellow light when stock < 5 (via tape_low)
LOW_THRESHOLD_LABEL = 5    # yellow light when stock < 5 (via label_low)

# INITIAL STOCK VALUES
# (what you start with at time 0)
MAX_CARTON_STOCK = 10      # initial carton stock
MAX_TAPE_STOCK   = 36      # initial tape stock
MAX_LABEL_STOCK  = 36      # initial label stock

# REFILL LEVELS (Option B: refill to 50 / 50 / 50)
REFILL_CARTON_LEVEL = 50.0
REFILL_TAPE_LEVEL   = 50.0
REFILL_LABEL_LEVEL  = 50.0
...
class SensorsComponent:
    ...
    def __init__(self, args):
        ...
        # (updated initial stock values)
        self.carton_stock = float(MAX_CARTON_STOCK)   # 10
        self.tape_stock = float(MAX_TAPE_STOCK)       # 36
        self.label_stock = float(MAX_LABEL_STOCK)     # 36
        ...
        # --------------- BOX POSITION / QUALITY PROCESS ----------------
        def box_position_process(env, signals):
            """
            Simple cyclic generator for:
            loader_pocket_carton_present, box_at_flap, box_at_tape, box_at_label,
            product_placed_ok, top_flaps_closed_ok, tape_applied_ok, label_applied_ok.

            This keeps all logic on the Sensors side, PLC only reads them.
            """
            while True:
                # stage 1: carton in pocket
                signals.loader_pocket_carton_present = 1
                signals.box_at_flap = 0
                signals.box_at_tape = 0
                signals.box_at_label = 0
                signals.product_placed_ok = 0
                signals.top_flaps_closed_ok = 0
                signals.tape_applied_ok = 0
                signals.label_applied_ok = 0
                yield env.timeout(2.0)

                # stage 2: at flap, flaps closed
                signals.loader_pocket_carton_present = 0
                signals.box_at_flap = 1
                signals.top_flaps_closed_ok = 1
                yield env.timeout(2.0)

                # stage 3: at tape, tape applied OK
                signals.box_at_flap = 0
                signals.box_at_tape = 1
                signals.tape_applied_ok = 1
                yield env.timeout(2.0)

                # stage 4: at label, label applied OK, product placed OK
                signals.box_at_tape = 0
                signals.box_at_label = 1
                signals.label_applied_ok = 1
                signals.product_placed_ok = 1
                yield env.timeout(2.0)

                # reset and repeat (box leaves)
                signals.box_at_label = 0
                signals.product_placed_ok = 0
                signals.top_flaps_closed_ok = 0
                signals.tape_applied_ok = 0
        ...
        self.env.process(printer_process(self.env, s))
        self.env.process(stock_flags_process(self.env, self, s))
        self.env.process(conveyor_process(self.env, self, s))

        # start SimPy failure processes (robot + machines)
        self.env.process(
            failure_process(self.env, s,
        ...
        # start box-position process
        self.env.process(box_position_process(self.env, s))
    ...
    def mainThread(self):
        ...
        try:
            ...
            while(vsiCommonPythonApi.getSimulationTimeInNs() < self.totalSimulationTime):
                ...
                #    IMPORTANT: if stock is 0 and a consume_cmd arrives,
                #    we interpret this as "new pallet/roll arrived" and
                #    refill ONLY that material to 50 (B option).
                if self.mySignals.carton_consume_cmd:
                    if self.carton_stock <= 0.0:
                        # refill carton to 50
                        self.carton_stock = REFILL_CARTON_LEVEL
                    else:
                ...
                if self.mySignals.tape_consume_cmd:
                    if self.tape_stock <= 0.0:
                        # refill tape to 50
                        self.tape_stock = REFILL_TAPE_LEVEL
                    else:
                        self.tape_stock -= 1.0
                ...
                if self.mySignals.label_consume_cmd:
                    if self.label_stock <= 0.0:
                        # refill label to 50
                        self.label_stock = REFILL_LABEL_LEVEL
                    else:
                        self.label_stock -= 1.0

                # NOTE:
                # - HR component simulates refill time (WAIT_HR_REFILL in PLC).
                # - After hr_refill_done, PLC resumes and sends the next
                #   consume_cmd. At that moment stock==0, so we jump to 50
                #   for that specific material only.
    ...
    def sendEthernetPacketToPLCComponent(self):
        ...
        # Order must match VSI definition
        ...
        # NEW sensor outputs (box state + quality), in the same order as VSI
