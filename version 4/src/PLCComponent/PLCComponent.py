...
class MySignals:
    def __init__(self):
        ...
        # NEW: command to sensors to refill stocks to level 50
...
class PLCComponent:
    ...
    def __init__(self, args):
        ...
        # One-shot rework guards
        self.flap_rework_attempted = False
        self.tape_rework_attempted = False
        self.label_rework_attempted = False
    ...
    def mainThread(self):
        ...
        try:
            ...
            while(vsiCommonPythonApi.getSimulationTimeInNs() < self.totalSimulationTime):
                ...
                # Clear rework guards after HR intervention
                if s.hr_repair_done:
                    self.flap_rework_attempted = False
                if s.hr_refill_done:
                    self.tape_rework_attempted = False
                ...
                # --- reset all "instant" outputs every cycle ---
                # consumptions are one-shot pulses
                s.carton_consume_cmd = 0
                s.tape_consume_cmd = 0
                s.label_consume_cmd = 0
                # NEW: refill_stocks_cmd is also a one-shot pulse
                s.refill_stocks_cmd = 0
                ...
                if stock_empty and self.state not in (
                    ...
                    # simple encoding: 1 = generic material refill
                ...
                if self.state == StationState.WAIT_HR_REPAIR:
                    ...
                    if s.hr_refill_done:
                        ...
                        s.hr_refill_type = 0
                        # NEW: tell Sensors to reset all stocks to 50
                        s.refill_stocks_cmd = 1
                        self.state = StationState.IDLE
                        self.state_time_s = 0.0
                    ...
                    # box at flap folding unit with one rework attempt
                    s.flap_folder_enable = 1

                    if self.state_time_s >= 1.0:
                        if s.top_flaps_closed_ok:
                            # success, move on
                            self.flap_rework_attempted = False
                            self.state = StationState.TAPE_SEAL
                            self.state_time_s = 0.0
                        elif not self.flap_rework_attempted:
                            # retry once
                            self.flap_rework_attempted = True
                            self.state_time_s = 0.0
                        else:
                            # second failure -> escalate to HR
                            s.hr_repair_request = 1
                            s.hr_repair_type = 2  # flap unit error code
                            self.state = StationState.WAIT_HR_REPAIR
                            self.state_time_s = 0.0

                elif self.state == StationState.TAPE_SEAL:
                    # box at tape unit with one rework attempt
                    ...
                    if self.state_time_s >= 1.0:
                        if s.tape_applied_ok:
                            self.tape_rework_attempted = False
                            self.state = StationState.LABEL_APPLY
                            self.state_time_s = 0.0
                        elif not self.tape_rework_attempted:
                            # retry once
                            self.tape_rework_attempted = True
                            self.state_time_s = 0.0
                        else:
                            # failed rework; proceed (refill path will handle if stock is empty)
                            self.state = StationState.LABEL_APPLY
                            self.state_time_s = 0.0

                elif self.state == StationState.LABEL_APPLY:
                    # box at label unit with one rework attempt
                    ...
                    if self.state_time_s >= 1.0:
                        if s.label_applied_ok:
                            self.label_rework_attempted = False
                            self.state = StationState.MOVE_OUT
                            self.state_time_s = 0.0
                        elif not self.label_rework_attempted:
                            # retry once
                            self.label_rework_attempted = True
                            self.state_time_s = 0.0
                        else:
                            # failed rework; proceed (refill path will handle if stock is empty)
                            self.state = StationState.MOVE_OUT
                ...
                print("\trefill_stocks_cmd =", end = " ")
                print(self.mySignals.refill_stocks_cmd)
    ...
    def sendEthernetPacketToSensorsComponent(self):
        ...
        # NEW: send refill_stocks_cmd to Sensors
        bytesToSend += self.packBytes('?', self.mySignals.refill_stocks_cmd)
